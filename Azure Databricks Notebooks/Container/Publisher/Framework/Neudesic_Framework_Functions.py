...
if purviewEnabled == 'Yes':
  ...
  auth = ServicePrincipalAuthentication(
      ...
      client_id = "1a03fade-294c-4c7d-bce9-15e5e9c92d49", 
      client_secret = "DYC.4rs~Ia_0O15QuxwQvU3NQyxu~Y09w-"
  )
...
if purviewEnabled == 'Yes':
  ...
  # Create a client to connect to your service.
    ...
    process_qn = "notebook:  Purview - Create Lineage Enrich " + tableName
...
def execute_analytics_stored_procedure_with_results(execsp):
  conn = build_sql_odbc_connection(dbserver, analyticsdbname, username, pwd)
  cursor = conn.cursor()
  conn.autocommit = True
  cursor.execute(execsp)
  rc = cursor.fetchall()
...
def execute_analytics_stored_procedure_no_results(execsp):
  conn = build_sql_odbc_connection(dbserver, analyticsdbname, username, pwd)
  conn.autocommit = True
  conn.execute(execsp)
  conn.close()
...
def execute_sqldw_stored_procedure_no_results(execsp):
  conn = build_sql_odbc_connection(sqldwservername, sqldwdatabasename, sqldwusername, sqldwpassword)
  conn.autocommit = True
  conn.execute(execsp)
  conn.close()
...
def build_sqldw_jdbc_url_and_connectionProperties(dbserver, dwname, username, pwd):
  ...
  return sqlDwUrlSmall, connectionProperties
...
def calculate_check_digit(giftcardnumber):
  try:
    checkDigit = 0
    summ = 0
    isEvenPlace = True
    chararray = giftcardnumber[::-1]
    for i in range(0, 17):
      numStr = str(chararray[i])
      num = int(numStr)
      if (isEvenPlace):
        num *= 2
        if (num > 9):
    ...
    return checkDigit
     ...
     return ""
print(calculate_check_digit("600488076000330762"))

# COMMAND ----------

def get_hash_sha256(text):
  salt = "c6rSEZrqWf8GLBgYRbzLqKgU"
  text = text + salt
  bytes = text.encode(encoding='UTF-8',errors='strict') 
  hashstr = str(hash(bytes)) 
  hashString = ""
  for b in hashstr:
    hashString += '0:x2' + str(b)
  return hashString
...
def getValueFromJson (myjson,path):
  if not myjson:
    if (myjson.find(path)!=-1):
      json_object = json.loads(myjson)
      return str(json_object[path])
    else:
      return
  else:
      return
...
print(getValueFromJson("","deliveryInstruction"))

# COMMAND ----------

import re

def convertStringToJsonOld(inputString):
    if len(inputString)>0:
      item = {}
      nameregex = "\"name\": \"(.*?)\""
      names = re.findall(nameregex,inputString)
      valuesregex = "\"value\": \"(.*?)\""
      values = re.findall(valuesregex,inputString)
      convertString = "{"
      for x,y in names,values:
          #{"Cust":"Original","Size":"Large"}
          convertString += "\"" + x + "\":\"" + y + "\","  
...
def convertStringToJson(inputString):
  if len(inputString)>0:
    item = {}
    nameregex = "\"name\": \"(.*?)\""
    names = re.findall(nameregex,inputString)
    ...
    try:
            idx=0
            for x in names:
                convertString += "\"" + x + "\":\"" + values[idx] + "\","
                idx=idx+1

    except:
            convertString += "\"" + names[0] + "\":\"" + values[0] + "\","
    convertString = convertString[:-1] + "}"
    return convertString
  else:
    return 
...
def getStringValue(stringToSearch, stringToSearchFor, stringToEndOn):
  returnValue = ""
  if (stringToSearch.index(stringToSearchFor) > 0):

    returnValue = stringToSearch[stringToSearch.index(stringToSearchFor) + len(stringToSearchFor): stringToSearch.index(stringToSearchFor) + len(stringToSearchFor) + stringToSearch.index(stringToEndOn, stringToSearch.index(stringToSearchFor) + len(stringToSearchFor)) - (stringToSearch.index(stringToSearchFor) + len(stringToSearchFor))]
...
import json
def extractStoreReplacementHistory(inputDescription):
    item = {}
    _bodyDescription = getStringValue(inputDescription, "<b>Body:</b>", "</div>");
    if (_bodyDescription.find("(") and _bodyDescription.find(")")):
      _bodyDescription = getStringValue(_bodyDescription,"(", ")")
      _bodyDescription = _bodyDescription.replace("??", " ")
      item['Description'] = _bodyDescription
    else:
      item['Description'] = ""
    item['Expires'] = getStringValue(inputDescription, "<b>Expires:</b>", "</div>")
    item['Effective Date'] = getStringValue(inputDescription, "<b>Effective Date:</b>", "</div>")
    item['Store'] = getStringValue(inputDescription, "<b>Store #:</b>", "</div>")
    item['Action'] = getStringValue(inputDescription, "<b>Action:</b>", "</div>")
