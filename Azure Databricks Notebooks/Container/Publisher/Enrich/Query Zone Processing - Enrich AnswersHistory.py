...
# MAGIC  
# MAGIC %md
# MAGIC # Query Zone Processing - Enrich AnswersHistory
# MAGIC 
...
dbutils.widgets.text(name="tableName", defaultValue="AnswersHistory", label="Table Name")
...
cleansed_df=cleansed_df.withColumn('TemplateValue',cleansed_df['TemplateValue'].substr(0,4000))
...
# MAGIC %sql
# MAGIC select 
# MAGIC Max(length(AnswerVersion)), 
# MAGIC Max(length(TemplateValue)),
# MAGIC Max(length(ValidFrom)), 
# MAGIC Max(length(ValidTo)), 
# MAGIC Max(length(Value))
# MAGIC from rsm.AnswersHistory
