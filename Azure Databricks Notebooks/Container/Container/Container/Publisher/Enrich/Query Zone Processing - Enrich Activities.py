...
# MAGIC  
# MAGIC %md
# MAGIC # Query Zone Processing - Enrich Activities
# MAGIC 
...
dbutils.widgets.text(name="tableName", defaultValue="Activities", label="Table Name")
...
databaseTableName = containerName.replace("-","") + "." + tableName
...
cleansed_df=cleansed_df.withColumn('PropertyBag',cleansed_df['PropertyBag'].substr(0,4000))

# COMMAND ----------

cleansed_df=cleansed_df.withColumn('Value',cleansed_df['Value'].substr(0,4000))
...
display(spark.sql("CREATE DATABASE IF NOT EXISTS " + containerName.replace("-","")))
...
destinationPath = enrichedPath.replace("abfss://" + containerName + "@","https://").replace("Query/Enriched", containerName + "/Query/Enriched") + "/{SparkPartitions}"
...
client.get_entity(qualifiedName=sourcePath,typeName="azure_datalake_gen2_resource_set")
