...
dbutils.widgets.text(name="tableName", defaultValue="Engagements", label="Table Name")
...
destinationTableName = 'Engagements'
...
notebookName = "Query Zone Processing - Append Engagements"
...
stgViewName =  "rsm.Incremental_Engagements"

sql="""
CREATE OR REPLACE VIEW {0} AS
SELECT 
CanPreparerContactClient
,CchVersion
,ClientContactEmail
,ClientContactName
,ClientContactPhone
...
,CompletionDate
,Complexity
,ConcurringReviewDone
,ConcurringReviewStarted
,CreationDate
,Creator
,CriticalError
,DeletedDate
...
,IsDeleted
,IsParentEntity
,IsPartOfConsolidatedReturn
,cast(LastQuestionId as Long) as LastQuestionId
,cast(LastSectionId as Long) as LastSectionId
,cast(LastUpdatedById as Long) as LastUpdatedById
,LastUpdatedDate
,string(hash('sha2_256',Name)) as Name
,NeedsReviewQuestions
,OcrFolderName
,PdfProcessingMethod
,Phase
,PreparerReviewDone
,PreparerReviewStarted
,PrimaryReviewDone
,PrimaryReviewStarted
,PriorYearReturnId
,RollForwardDate
,SecondaryReviewDone
,SecondaryReviewStarted
,SuccessfulQuestions
,TaxType
,TaxYear
,TotalExpectedUnits
...
,UnansweredQuestions
,UseUnits
,UsedLastYearsAnswers
,dateLoaded
,'Query Zone Processing - Append Engagements' AS Created_By
,'' AS Modified_By
...
# MAGIC %sql
# MAGIC 
# MAGIC MERGE INTO rsm.Engagements
# MAGIC USING rsm.Incremental_Engagements
# MAGIC ON rsm.Engagements.Id = rsm.Incremental_Engagements.Id
...
stageTableName = "Incremental_Engagements"
...
execsp = "DELETE rsm.Engagements FROM rsm.Engagements e JOIN stage.Incremental_Engagements i ON e.Id = i.Id"
try:
...
""".format("rsm.Incremental_Engagements")
spark.sql(sql)
...
execsp = "IF OBJECT_ID('stage.Incremental_Engagements') IS NOT NULL DROP TABLE stage.Incremental_Engagements"
execute_sqldw_stored_procedure_no_results(execsp)
