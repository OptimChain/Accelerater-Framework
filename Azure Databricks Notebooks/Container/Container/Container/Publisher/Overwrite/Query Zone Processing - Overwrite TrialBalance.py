...
# MAGIC  
# MAGIC %md
# MAGIC # Query Zone Processing - Overwrite TrialBalance
# MAGIC ###### Author: Mike Sherrill 04/23/2021
# MAGIC 
...
cleansed_df = cleansed_df \
  .withColumnRenamed('CTESTCLIENT', 'Account') \
  .withColumnRenamed('T101', 'PctChange') \
  .withColumnRenamed('Unnamed1', 'Prelim') \
  .withColumnRenamed('Unnamed2', 'Adjustment') \
  .withColumnRenamed('Unnamed3', 'Reclass') \
  .withColumnRenamed('Unnamed4', 'Rep') \
  .withColumnRenamed('Unnamed5', 'Annotation') \
  .withColumnRenamed('Unnamed6', 'RepDate') \
  .drop('T104')

# COMMAND ----------

cleansed_df.registerTempTable('stgData')
...
# COMMAND ----------
  ...
  SELECT *
  FROM {0}
  WHERE Account NOT LIKE 'Year End%'
  AND Account NOT LIKE '%NaN%'
  """.format('stgData')
