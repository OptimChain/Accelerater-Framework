...
notebookName = "Query Zone Processing - InferSchema"
...
"""
check existence of actual file and set fullDataPath variable if found
"""
onlyAFile = fullDataPath + "/" + tableName + ".json"
try:
  specificFile = dbutils.fs.ls(onlyAFile)
except Exception as e:
  specificFile = "Folder"

if specificFile != "Folder":
  tableName = tableName + ".json"
  fullDataPath = onlyAFile
else:
...
dbutils.fs.ls(fullDataPath)

# COMMAND ----------

def validate_raw_data_path(DataPath):
  try:
...
try:
  ...
  sourceName = "Query Zone Processing - InferSchema: Attempt to Sample Source and Infer Schema"
  errorCode = "200"
...
try:
  ...
  sourceName = "Query Zone Processing - InferSchema: Convert Schema to JSON and Save to Data Lake"
  errorCode = "400"
  errorDescription = e.message
  log_event_notebook_error(notebookExecutionLogKey, sourceName, errorCode, errorDescription)
  raise(e)
