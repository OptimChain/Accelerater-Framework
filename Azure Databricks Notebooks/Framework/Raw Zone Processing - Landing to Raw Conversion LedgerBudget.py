...
# MAGIC  
# MAGIC %md
# MAGIC # Raw Zone Processing - Landing to Raw Conversion LedgerBudget
# MAGIC ###### Author: Shubham S 12/24/2020
# MAGIC 
# MAGIC #### Usage
...
dbutils.widgets.text(name="externalDataPath", defaultValue="/Landing/LedgerBudget", label="External Data Path")
...
if any(mount.mountPoint == mountedPath+"/" for mount in dbutils.fs.mounts()):
...
print(mountedPathPrefix+mountedLandingPath)
...
try:
  ...
  def convertExcelToJson(file,destinationFileName,sheetName):
...
def fileConversion(sourceFile,destinationFile,fileExtension,sheetName):
  if fileExtension == 'xlsx' or fileExtension == 'xls':
    print("found Excel")
    convertExcelToJson(sourceFilePath, destinationFilePath,sheetName)
    ...
    if len(fileList)>0:
      ...
      for fname in fileList:
          ...
          for sheetName in sheets:
            ...
            #createDir = dirName.replace(mountedPathPrefix,'',1).replace("/Landing" , "/Raw" , 1) + "/" + fileName        
            createDir = dirName.replace(mountedPathPrefix,'',1).replace("/Landing" , "/Raw" , 1) + "/" + sheetName
            dbutils.fs.mkdirs(createDir)
            destinationFilePath = mountedPathPrefix+createDir +"/"+ sheetName +'.json'
            fileConversion(sourceFilePath,destinationFilePath,fileExtension,sheetName)
